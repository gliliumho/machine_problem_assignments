...
./matmult1 [-m ROWS] [-n COLS] [-t NO_THREADS]
...
void parse_args(int argc, char *argv[])
{
    ...
    for (i=0; i<argc; i++){
        if (strcmp(argv[i],"-n") == 0 && (i < argc-1))
            ...
            printf("usage: ./matmult1 [-m ROWS] [-n COLS] [-t NO_THREADS]\n");
            exit(0);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    printf("m = %d, n = %d, no_threads = %d\tpart1b: OpenMP(row first)\n", array_m, array_n, no_threads);
